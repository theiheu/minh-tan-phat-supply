...
1. **`(auth)`**: Các trang công khai phục vụ xác thực người dùng (Đăng nhập, Quên mật khẩu).
2. **`(app)`**: Toàn bộ không gian làm việc chính sau khi đăng nhập (Được bảo vệ bởi Middleware xác thực phiên GoTrue).
3. **`api/`**: Các Endpoint xử lý xuất file Excel và upload hình ảnh.
...
|---|---|---|---|
| `/dashboard` | **Tổng quan & Nhật ký** | Dashboard hoạt động gần nhất, thông báo, nhật ký kiểm toán. | Tất cả vai trò đã đăng nhập |
| `/products` | **Tra cứu Danh mục** | Xem danh mục sản phẩm, biến thể, giá bán, vị trí kệ và tồn kho khả dụng. | Tất cả vai trò |
...
| `/receipts/[id]/edit`| **Sửa Phiếu Nhập** | Chỉnh sửa phiếu nhập kho đang ở trạng thái `draft`. | `superuser`, `owner`, `accountant`, `warehouse` |
...
| `/admin/products`| **Quản Trị Vật Tư** | Tạo sản phẩm, cấu hình biến thể, thiết lập quy đổi đơn vị (Thùng/Hộp/ml). | `superuser`, `owner`, `accountant`, `warehouse` |
...
| `/admin/vehicles` | **Dàn Xe Cơ Giới** | Danh mục xe ben, xe xúc, máy phát điện, cấu hình định mức và in tem QR dán xe. | `superuser`, `owner`, `accountant`, `warehouse` |
...
|---|---|---|
| `/api/reports/export` | `GET` / `POST` | Xuất báo cáo tài chính, báo cáo XNT hoặc báo cáo tiêu hao xe ra định dạng Excel (`.xlsx`). |
