...
erDiagram
    ...
    PRODUCTS ||--o{ VARIANTS : has_variants
    VARIANTS ||--o{ VARIANT_COMPONENTS : kit_parts
    VARIANTS ||--o{ STOCK_BALANCES : balances
    STOCK_LOCATIONS ||--o{ STOCK_BALANCES : holds
    VARIANTS ||--o{ STOCK_MOVEMENTS : movements
    STOCK_LOCATIONS ||--o{ STOCK_MOVEMENTS : location_moves

    SUPPLIERS ||--o{ RECEIPTS : supplies
    RECEIPTS ||--o{ RECEIPT_ITEMS : contains
    VARIANTS ||--o{ RECEIPT_ITEMS : received_variant

    CUSTOMERS ||--o{ ISSUES : buys
    ISSUES ||--o{ ISSUE_ITEMS : contains
    VARIANTS ||--o{ ISSUE_ITEMS : issued_variant

    PROFILES ||--o{ REQUISITIONS : created_by
    REQUISITIONS ||--o{ REQUISITION_ITEMS : contains
    VARIANTS ||--o{ REQUISITION_ITEMS : requested_variant
    ...
    VARIANTS ||--o{ DEFECT_NOTE_ITEMS : broken_variant
    ...
    VARIANTS ||--o{ TOOL_BORROWING_ITEMS : borrowed_tool
    ...
    VARIANTS ||--o{ STOCKTAKE_ITEMS : counted_variant
...
13. **`stock_balances`**: Tồn kho khả dụng thời gian thực (`location_id`, `variant_id`, `quantity`).
14. **`stock_movements`**: Sổ cái ghi nhận 100% biến động tăng giảm kho (`location_id`, `variant_id`, `quantity_change`, `movement_type`, `reference_type`, `reference_id`, `balance_after`, `created_by`).
...
16. **`receipt_items`**: Chi tiết hàng nhập (`receipt_id`, `variant_id`, `quantity`, `unit_cost`, `lot_number`, `expiry_date`).
...
18. **`issue_items`**: Chi tiết hàng xuất (`issue_id`, `variant_id`, `quantity`, `unit_price`).
...
20. **`requisition_items`**: Chi tiết vật tư yêu cầu (`requisition_id`, `variant_id`, `quantity_requested`, `quantity_issued`).
...
22. **`requisition_return_items`**: Chi tiết vật tư trả lại (`return_id`, `variant_id`, `quantity`).
...
24. **`defect_note_items`**: Chi tiết thiết bị hỏng (`defect_note_id`, `variant_id`, `quantity`, `damage_type`, `severity`, `images`, `resolution`: `repaired`/`liquidated`).
...
32. **`tool_borrowing_items`**: Chi tiết dụng cụ mượn (`borrowing_id`, `variant_id`, `quantity`, `returned_quantity`).
...
39. **`stocktake_items`**: Chi tiết đếm kiểm kê (`session_id`, `variant_id`, `system_qty`, `actual_qty`, `difference`, `reason`).
...
* **Tra cứu tồn kho:** `idx_stock_balances_loc_var` (`location_id, variant_id`).
* **Sổ cái thời gian thực:** `idx_stock_movements_variant_time` (`variant_id, created_at DESC`).
